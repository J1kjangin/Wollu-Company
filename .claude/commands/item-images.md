...
```
이 스크립트가 하는 일:
- 여백(투명/단색 배경) 자동 크롭 → 정사각 캔버스 중앙 정렬(6% 패딩) → **512px / 128px 두 벌** LANCZOS 리사이즈
- 최적화 저장(RGBA optimize vs 256색 양자화 중 작은 쪽)
  → `itemImages/item_<번호>.png` (512px, 무대용) + `itemImages/thumb/item_<번호>.png` (128px, 목록용)
...
- 개발자 콘솔에서 상태 인스턴스(`window.Workspace.state`)로 해당 아이템을 구매·배치한 뒤 확인한다:
  - 배치된 아이템에 `<img class="cy-item-img">` 가 생기고 `naturalWidth === 512`(무대) / `128`(목록 카드),
    onerror 폴백이 발동하지 않을 것
...
- 브라우저 캐시가 남으면 `index.html` 의 `ITEM_IMAGE_VERSION` 을 올린다.
- 주의: 스크립트는 `itemImages/` 에 있는 모든 `item_*.png` 를 `ITEM_IMAGE_IDS` 에 등록한다.
  아직 노출하지 않을 아이템 원본은 `assets/items/` 에 두지 않는다.
